...
### 4-1. Telegram 봇 준비 (사전 작업)

`TELEGRAM_BOT_TOKEN`과 `TELEGRAM_CHAT_ID`는 스크립트 실행 전에 미리 발급받아야 합니다.  
Telegram 알림이 필요 없으면 이 단계를 건너뛰어도 됩니다 — 미설정 시 mock 로그로 대체됩니다.

**BOT_TOKEN 발급:**

1. Telegram 앱에서 `@BotFather`를 검색하여 대화를 시작합니다.
2. `/newbot` 명령을 입력합니다.
3. 봇 이름(표시 이름)과 사용자명(`_bot`으로 끝나야 함)을 순서대로 입력합니다.
4. BotFather가 응답한 메시지에서 토큰을 복사합니다.
   ```
   예시: 7123456789:AAFxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
   ```

**CHAT_ID 확인:**

1. 위에서 만든 봇을 Telegram에서 검색하여 `/start` 메시지를 보냅니다.
2. 아래 URL을 브라우저에서 열어 `chat.id` 값을 확인합니다 (`<YOUR_TOKEN>`을 실제 토큰으로 교체):
   ```
   https://api.telegram.org/bot<YOUR_TOKEN>/getUpdates
   ```
3. 응답 JSON에서 `"chat":{"id":` 뒤의 숫자를 복사합니다.
   ```json
   {"ok":true,"result":[{"message":{"chat":{"id":123456789,...}}}]}
   ```

> **그룹 채팅**을 사용하는 경우: 봇을 그룹에 추가하고 그룹 내에서 메시지를 보낸 뒤 같은 방법으로 chat ID를 확인합니다. 그룹 chat ID는 음수(`-100...`)입니다.
...
| 키 | 설명 | 예시 |
|---|---|---|
| `TELEGRAM_BOT_TOKEN` | 4-1에서 발급받은 봇 토큰 | `7123456789:AAFxxx...` |
| `TELEGRAM_CHAT_ID` | 4-1에서 확인한 chat ID | `123456789` |
| `MQTT_HOST` | Mac mini의 LAN IP 주소 | `192.168.1.100` |
...
### 4-1. Prepare a Telegram bot (prerequisite)

`TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID` must be obtained before running the scripts.  
This step is optional — if unset, the system falls back to a mock notification log.

**Get a BOT_TOKEN:**

1. Open Telegram and search for `@BotFather`.
2. Send `/newbot`.
3. Enter a display name, then a username ending in `_bot`.
4. Copy the token from BotFather's response.
   ```
   Example: 7123456789:AAFxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
   ```

**Find your CHAT_ID:**

1. Search for your new bot in Telegram and send it `/start`.
2. Open the following URL in a browser (replace `<YOUR_TOKEN>` with the actual token):
   ```
   https://api.telegram.org/bot<YOUR_TOKEN>/getUpdates
   ```
3. Copy the number after `"chat":{"id":` in the JSON response.
   ```json
   {"ok":true,"result":[{"message":{"chat":{"id":123456789,...}}}]}
   ```

> **Group chat**: Add the bot to a group, send a message in the group, then use the same URL to find the chat ID. Group IDs are negative numbers (`-100...`).

### 4-2. Write environment variables
...
|---|---|---|
| `TELEGRAM_BOT_TOKEN` | Bot token obtained in 4-1 | `7123456789:AAFxxx...` |
| `TELEGRAM_CHAT_ID` | Chat ID obtained in 4-1 | `123456789` |
| `MQTT_HOST` | Mac mini LAN IP address | `192.168.1.100` |

### 4-3. Configure Mosquitto
...
### 4-4. Configure Ollama and download model
...
### 4-5. Initialize SQLite database
...
### 4-6. Deploy policy and schema files
...
### 4-8. Configure Home Assistant (optional)
