...
POLICY_RUNTIME_DIR="${WORKSPACE_DIR}/docker/volumes/app/config/policies"
SCHEMA_RUNTIME_DIR="${WORKSPACE_DIR}/docker/volumes/app/config/schemas"
MQTT_RUNTIME_DIR="${WORKSPACE_DIR}/docker/volumes/app/config/mqtt"
PAYLOAD_RUNTIME_DIR="${WORKSPACE_DIR}/docker/volumes/app/config/payloads"
...
mkdir -p "${SCHEMA_RUNTIME_DIR}"
mkdir -p "${MQTT_RUNTIME_DIR}"
mkdir -p "${PAYLOAD_RUNTIME_DIR}"
...
append_env "${PYTHON_ENV_FILE}" "POLICY_DIR" "${POLICY_RUNTIME_DIR}"
append_env "${PYTHON_ENV_FILE}" "SCHEMA_DIR" "${SCHEMA_RUNTIME_DIR}"
append_env "${PYTHON_ENV_FILE}" "MQTT_REGISTRY_DIR" "${MQTT_RUNTIME_DIR}"
append_env "${PYTHON_ENV_FILE}" "PAYLOAD_EXAMPLES_DIR" "${PAYLOAD_RUNTIME_DIR}"
