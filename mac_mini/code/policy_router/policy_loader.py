"""
frozen 정책 파일 로더.
임계값, 트리거 조건, 타임아웃 등을 절대 하드코딩하지 않는다.
라우터가 필요한 모든 정책 값은 이 모듈을 통해 읽어온다.
"""
import json
from pathlib import Path
from typing import Any, Dict

# mac_mini/code/policy_router/policy_loader.py 기준 4단계 상위가 리포지토리 루트
...
def load_policy_table() -> Dict[str, Any]:
    """policy_table_v1_1_2_FROZEN.json 을 읽어 dict로 반환한다."""
    with open(POLICY_TABLE_PATH, encoding="utf-8") as f:
        return json.load(f)


def load_low_risk_actions() -> Dict[str, Any]:
    """low_risk_actions_v1_1_0_FROZEN.json 을 읽어 dict로 반환한다."""
