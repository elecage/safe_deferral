...
이 문서는 `integration/scenarios/`에 있는 scenario skeleton들을 **사람이 실제 사용 맥락과 연결해서 검토하기 쉽게 설명**하기 위한 안내서다.
...
- **중증 신체 장애인 및 운동 기능 저하 사용자**  
  예: 뇌성마비, 근육병, 척수 손상, 뇌졸중 등으로 손가락 미세 조작이나 이동이 어려운 사람
- **언어 및 발화 제약이 있는 사용자**  
  예: 조음장애, 피로, 호흡 문제 등으로 일반 음성비서가 요구하는 또렷한 발화가 어려운 사람
- **일상 기능 저하를 겪는 고령층**  
  예: 근력, 시력, 반응속도, 인지 기능 저하로 스마트홈 제어와 안전 관리가 어려운 사람
- **보호자 및 활동지원사**  
  Safe Deferral 또는 Caregiver Escalation 이후 제한적으로 개입하는 2차 사용자

따라서 scenario는 단순히 “기술적으로 어떤 입력이 들어왔다”만 보면 부족하다.  
반드시 **이 입력이 실제 생활에서 어떤 몸의 조건, 어떤 집안 상황, 어떤 위험에서 나오는가**를 함께 생각해야 한다.

---

## 2. 시나리오를 검토할 때 기본적으로 볼 것

각 시나리오를 검토할 때는 아래 여섯 가지를 우선 본다.
...
#### 실제 생활 맥락
이 파일은 직접 특정 한 사람의 생활 상황을 대표한다기보다, 앞으로 추가할 현실적 시나리오의 출발점이다.
...
### 3.3 `class1_baseline_scenario_skeleton.json`
...
### 3.4 `class2_insufficient_context_scenario_skeleton.json`
...
보호자에게 넘기거나, bounded clarification이 가능한 단계로 가야 한다.
...
- 실제 생활에서는 “모르는 상태에서 그냥 실행하면 곤란한 상황”으로 이해되는가

---

### 3.5 `stale_fault_scenario_skeleton.json`
...
### 3.6 `conflict_fault_scenario_skeleton.json`
...
예를 들면:
- 한 번의 버튼 입력만으로는 조명을 켜고 싶은지, 경고 출력을 원한 것인지 확실하지 않은 상황
...
이건 단순 기술 문제를 넘어서, **입력이 적은 사용자를 존중하는 방식**과도 연결된다.  
입력이 적다는 이유로 시스템이 자의적으로 의미를 채워 넣으면, 오히려 접근성이 해쳐진다.
...
### 3.7 `missing_state_scenario_skeleton.json`
