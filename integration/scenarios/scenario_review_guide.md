...
- doorlock-sensitive request는 Class 1 autonomous execution이 아니라 Class 2 escalation 또는 governed manual confirmation path로 해석해야 한다.
...
#### 기대되는 안전한 결과
- `CLASS_2` 또는 safe deferral / clarification-compatible outcome으로 보수적 처리
- LLM decision/candidate generation은 금지되거나 제한됨
- user-facing guidance generation은 policy-constrained explanation으로 허용될 수 있음
- unsafe autonomous actuation이 허용되지 않음
...
- `llm_invocation_allowed=false`가 decision invocation 금지인지, guidance까지 금지하는지 문서/JSON에서 명확한가
...
6. scenario topic alignment verifier 추가
7. fixture reference existence verifier 추가
8. policy/schema alignment verifier 추가
