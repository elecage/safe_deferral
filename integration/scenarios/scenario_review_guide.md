...
정책/스키마/용어의 authoritative baseline은 `common/`의 canonical assets에 남는다.
...
- scenario는 canonical policy, canonical schema, MQTT topic registry, interface matrix를 소비한다.
...
- Class 1 autonomous low-risk execution은 현재 canonical lighting catalog로 제한된다.
...
- canonical policy/schema를 소비하는 구조를 유지하는가
- `low_risk_actions.json` 밖의 action을 Class 1 autonomous execution으로 표현하지 않는가
...
- ingress topic이 `common/history/mqtt/topic_registry.json`에 있는가
...
- unsafe autonomous actuation이 허용되지 않음
- autonomous execution 범위는 canonical lighting catalog 안에 머묾
...
- Class 1 expected action이 `low_risk_actions.json` 안에 있는가
...
- exact stale predicate를 scenario가 직접 정의하지 않고 canonical assets에 맡기고 있는가
...
- missing-state를 scenario가 임의 규칙으로 정의하지 않고 canonical schema와 연결할 수 있는가
...
- Class 1 scenario가 canonical lighting catalog 밖으로 나가지 않는가
...
- exact threshold는 scenario 안이 아니라 canonical assets에서 해석되어야 함
