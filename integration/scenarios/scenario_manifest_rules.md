...
### 5.1 Ordinary context/input scenario
...
### 5.2 Class 0 emergency scenario
...
## 7. expected_outcomes 필드 규칙

`expected_outcomes`는 scenario의 성격에 따라 두 가지 스타일 중 하나를 택할 수 있다.

### 7.1 deterministic single-outcome style
...
### 7.2 allowed/prohibited outcome style
...
### 7.3 LLM invocation field interpretation
...
## 8. Class 1 low-risk boundary 규칙
...
- Some skeletons may still contain legacy `smarthome/...` topics. These should be replaced in the JSON alignment phase.
...
1. scenario skeleton JSON topic alignment
2. scenario loader unit test 추가
3. expected outcome comparator 추가
4. `scenario_manifest_schema.json` 추가
5. `verify_scenario_topic_alignment.py` 추가
6. `verify_scenario_fixture_refs.py` 추가
7. `verify_scenario_policy_schema_alignment.py` 추가
