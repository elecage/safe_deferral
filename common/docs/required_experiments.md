...
### 8.6 LLM-driven candidate generation quality (`class2_llm_quality` 서브블록)

`09_llm_driven_class2_candidate_generation_plan.md`의 Phase 5에서 도입된 LLM-driven Class 2 candidate generation의 운영 품질을 측정하기 위해, Package D의 `_metrics_d` 결과는 `class2_llm_quality` 서브블록을 함께 보고한다. 이 서브블록은 RPi 측 `ClarificationStore`에 캡처된 `safe_deferral/clarification/interaction` 레코드의 `candidate_source` 필드(`llm_generated` | `default_fallback`)를 기준으로 산출된다.

| Metric | 의미 |
|---|---|
| `clarification_record_count` | 캡처된 clarification record 수 (서브블록 비율 분모) |
| `llm_generated_count` / `llm_generated_rate` | LLM 어댑터가 bounded-variability + catalog 게이팅을 통과한 후보를 생성하여 매니저가 채택한 세션 비율 |
| `default_fallback_count` / `default_fallback_rate` | 정적 `_DEFAULT_CANDIDATES` 테이블이 사용된 세션 비율 (LLM 미사용, LLM 거부, 또는 `pure_context_payload` 부재 — 예: C205 ACK timeout escalation) |
| `llm_user_pickup_rate` | LLM 세션 중 `selection_result.confirmed=true`로 사용자/보호자가 후보를 선택한 비율 |
| `default_fallback_user_pickup_rate` | 정적 fallback 세션의 같은 비율 (직접 비교 baseline) |

**원래 Phase 5 metric 제안과의 차이:** `09_*` 문서가 처음 제안한 `llm_candidate_admissibility_rate`, `prompt_length_violation_rate`, `llm_candidate_relevance_rate`는 Phase 1+2에서 어댑터가 catalog 외 후보·길이 초과 prompt·알 수 없는 candidate id를 매니저에 도달하기 전에 모두 사전 거부하므로 항상 100% 또는 0%로 떨어지는 구조적 문제가 있었다. 위 4종 metric은 이 사전 게이팅과 무관하게 의미 있는 값을 측정한다. 자세한 사유는 `SESSION_HANDOFF_2026-05-01_LLM_DRIVEN_CLASS2_PHASE5_LLM_QUALITY_METRICS.md` 참조.
