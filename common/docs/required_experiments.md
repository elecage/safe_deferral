...
실험 입력은 `context_schema_v1_0_0_FROZEN.json`을 만족해야 한다. 즉, 다음 세 블록이 모두 존재해야 한다.
...
Class 1 low-risk path에서 LLM 또는 bounded assistance layer가 생성하는 후보는 `candidate_action_schema_v1_0_0_FROZEN.json`을 만족해야 한다. 후보는 다음 두 부류만 허용한다.
