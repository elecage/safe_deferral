...
|---|---|---|---|---|
| User input | **Bounded Input Node** | Detect button input, debounce input, distinguish single input / repeated input / triple-hit input, collect candidate selection input | ESP32, Raspberry Pi Pico W, push button, large adaptive button, tact switch, pull-up/pull-down resistor, enclosure | Baseline, Class 1, Class 2, E002, Conflict Fault, Missing-State |
...
| Lighting actuation | **Lighting Actuator Node** | Turn lights on/off, report current lighting state, return execution ACK | ESP32, relay module, SSR, smart plug, Matter light, LED light, MOSFET module | Baseline, Class 1, Class 2-to-Class 1, Conflict Fault, Missing-State |
...
| Emergency aggregation | **Emergency Node** | Generate and publish emergency events from high temperature, smoke, gas, fall, or triple-hit evidence | ESP32/Pico W, integrated sensor board, MQTT emergency publisher | E001, E002, E003, E004, E005, Class 2-to-Class 0 |
...
| Voice input | **Voice Input Node** | Capture short user responses such as “1번”, “조명”, “긴급”, or “괜찮아요” | USB microphone, ReSpeaker mic array, smart-speaker bridge, Mac mini external microphone | Class 2, Conflict Fault, E005 |
| Voice output | **TTS/Voice Output Node** | Announce execution results, candidate choices, emergency warnings, and status checks | USB speaker, 3.5 mm speaker, Bluetooth speaker, smart speaker, DFPlayer Mini | All scenarios |
...
| Warning output | **Warning Output Node** | Provide warning sound, beacon, LED, vibration, or other urgent alert output | Buzzer, siren, LED strip, beacon light, vibration motor, relay module | E001, E002, E003, E004, E005 |
...
|---|---|---|---|---|
| Central runtime | **Mac mini Edge Hub** | Receive MQTT messages, aggregate context, perform routing, call LLM guidance, coordinate validator, dispatch commands, write audit records | Mac mini runtime, Python/Node service, Docker/venv | All scenarios |
...
| Input pattern analysis | **Input Pattern Detector** | Classify single input, repeated input, triple-hit, and candidate-selection input patterns | ESP32/Pico W firmware or Mac mini module | E002, Class 2, Conflict Fault |
...
| Class 2 interaction | **Class 2 Clarification Manager** | Present candidates, collect user selection, handle timeout, and manage transition to Class 0 / Class 1 / safe deferral | Mac mini software module | Class 2 |
| Candidate generation | **LLM Guidance Layer** | Generate bounded candidate choices and user-friendly guidance prompts | Mac mini local LLM, API-based LLM, prompt templates | Class 2, Conflict Fault |
| Policy classification | **Policy Router** | Classify inputs into Class 0, Class 1, Class 2, or fault-handling paths | Mac mini software module | All scenarios |
...
| Audit | **Audit Log** | Record input, candidates, decisions, transitions, execution, deferral, and notification outcomes | JSONL log, SQLite, file-based audit, dashboard integration | All scenarios |
...
| `class2_insufficient_context_scenario_skeleton.json` | Bounded Input Node, Context Node, Device State Reporter Node, TTS/Voice Output Node or Display Output Node | Mac mini Edge Hub, MQTT Broker, Class 2 Clarification Manager, LLM Guidance Layer, Policy Router, Audit Log |
| `conflict_fault_scenario_skeleton.json` | Bounded Input Node, Context Node, Occupancy/Location Node, Lighting Actuator Node, Device State Reporter Node, TTS/Voice Output Node or Display Output Node | Mac mini Edge Hub, MQTT Broker, Input Context Mapper, LLM Guidance Layer, Policy Router, Deterministic Validator, Audit Log |
| `missing_state_scenario_skeleton.json` | Bounded Input Node, Context Node, Device State Reporter Node, Device Health Reporter Node, TTS/Voice Output Node or Display Output Node | Mac mini Edge Hub, MQTT Broker, Health Check Routine, Policy Router, Deterministic Validator, Audit Log |
...
→ Class 1 bounded assistance, Class 0 emergency handling, or safe deferral / caregiver confirmation
```

### 5.3 Fault-scenario implication
...
Both should lead to conservative handling rather than unsafe autonomous execution.
...
8. E002 triple-hit emergency request
9. E001/E003/E004 environmental emergency sensors
10. E005 fall detection
```

The recommended strategy is to stabilize lighting assistance first, then verify missing-state and conflict behavior, then add Class 2 clarification, and finally attach emergency-sensor scenarios.
