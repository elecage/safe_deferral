...
- Treat Raspberry Pi 5 as an **evaluation-side dashboard, simulation, orchestration, replay, fault-injection, and result-artifact node**, not as a target for hub-side operational runtime configuration.
- Treat Raspberry Pi dashboard/governance tooling as non-authoritative inspection and validation support.
...
- `rpi/` contains dashboard, simulation, orchestration, replay, fault-injection, result-export, and synchronized runtime/reference assets
...
- ensure dashboard/governance topics cannot publish control authority unless explicitly allowed by the registry and runtime mode
...
- Topic ACLs should not accidentally grant dashboard/governance tooling policy, validator, caregiver approval, or actuation authority
...
### Role Boundary
Raspberry Pi 5 is a **dashboard, simulation, fault-injection, replay, orchestration, result-artifact, and evaluation node**.
...
Recommended responsibilities:
- configure MQTT/payload governance inspector runtime
...
- Dashboard/governance tooling may inspect, validate, visualize, and report, but must not override policy routing, validator decisions, caregiver approval, or dispatch.
...
- `rpi/scripts/configure/` configures the dashboard, simulation, orchestration, replay, fault-injection, result-export, and evaluation node only
...
- runtime correctness, topic/payload consistency, and canonical asset consistency are confirmed later in the verify stage
