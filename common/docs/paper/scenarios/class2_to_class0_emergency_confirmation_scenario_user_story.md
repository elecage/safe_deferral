...
- 이 경우 시스템은 즉시 긴급 안내(announce_emergency)를 수행하고 보호자에게 알림을 전송하는 Class 0 흐름으로 직접 전이한다 (Policy Router는 다시 호출되지 않는다).
```
...
| Class 0 전이 핸들러 | Mac mini Edge Hub | 확인 또는 증거에 따라 즉시 긴급 안내·보호자 알림 디스패치 (Policy Router 재호출 없음) | Class 0 흐름 직접 진입 |
...
| Class 0 전이 핸들러 | 긴급 확인 또는 결정론적 긴급 증거 도착 시 즉시 긴급 안내와 보호자 알림 발송 | Class 0 흐름으로 직접 전이 (Policy Router 재호출 없음) |
...
### 6.5 Class 0 전이 핸들러가 직접 긴급 흐름으로 진입한다

긴급 확인 또는 결정론적 증거는 Mac mini Edge Hub의 Class 0 전이 핸들러가 직접 처리한다 (Policy Router는 다시 호출되지 않는다).

이 단계에서 시스템은 긴급 전이가 후보 문장 때문이 아니라 실제 확인 또는 결정론적 증거 때문임을 audit log에 명시적으로 기록해야 한다.

### 6.6 Class 0 긴급 흐름으로 전환한다

Class 0 조건이 충족되면 시스템은 긴급 안내(announce_emergency)와 보호자 알림을 즉시 발송하는 긴급 대응 흐름으로 전환한다.
...
처리 경로: Mac mini Edge Hub의 Class 0 전이 핸들러가 즉시 긴급 안내·보호자 알림 디스패치 (Policy Router 재호출 없음)
